...
marked.use({ renderer: {
	blockquote: function(html) {
		...
		else return `<blockquote>HI MOM ${html}</blockquote>`;
	}
} });
...
module.exports = {
	...
	preloadBlog(callback) {
		...
		async.eachLimit( this.config.articles, 8,
	...
	handler: function(args, callback) {
		...
		if (uri.match(/^\/blog\/(.+?)\/?$/)) {
			...
			// latest blog article
			var slug = this.config.articles[0];
	...
	handle_feed: function(args, callback) {
		// rss feed
		var self = this;
		var latest_slug = this.config.articles[0];
		...
		this.config.articles.forEach( function(slug) {
			var article = self.articles[slug];
			var author = self.config.authors[ article.author ];
	...
	handle_doc: function(repo, path, args, callback) {
		// show github repo markdown file as page content
		var self = this;
		var page = this.config.pages[repo];
